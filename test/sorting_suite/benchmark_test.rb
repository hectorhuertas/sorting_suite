...
class BenchmarkTest < Minitest::Test
  ...
  def test_time_returns_sorting_time
    benchmark = SortingSuite::Benchmark.new
    returned = benchmark.time(SortingSuite::MergeSort)
    ...
    returned_short = benchmark.time(SortingSuite::MergeSort, [9, 5, 2, 1, 7])
    returned_long = benchmark.time(SortingSuite::MergeSort, ([9, 5, 2, 1, 7] * 50))
    assert returned_short.scan(/[0-9]./).join.to_f < returned_long.scan(/[0-9]./).join.to_f

    # longer time for longer arrays with BubbleSort
    returned_short = benchmark.time(SortingSuite::BubbleSort, [9, 5, 2, 1, 7])
    returned_long = benchmark.time(SortingSuite::BubbleSort, ([9, 5, 2, 1, 7] * 50))
    assert returned_short.scan(/[0-9]./).join.to_f < returned_long.scan(/[0-9]./).join.to_f
  end
  ...
  def test_fastest_and_slowest_has_correct_formatting
    benchmark = SortingSuite::Benchmark.new([4, 3, 6, 1])

    assert_equal 'is the fastest', benchmark.fastest.split.slice(1..-1).join(' ')
    assert_equal 'is the slowest', benchmark.slowest.split.slice(1..-1).join(' ')
  end
end
